...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \draw (0,0) grid (5, 1);
    \node at (-0.5, 0.5) {$\pi$};
    \node at (4.5, 0.5) {\scriptsize Cheese};
    \foreach \x in {0.5, ..., 3.5} {
      \draw[->] (\x,0.5) -- (\x-0.125,0.5);
      \draw[->] (\x,0.5) -- (\x+0.125,0.5);
    }
    % Updating Q(s,a)
    \draw (0,-2) grid (5, -1);
    \node at (-0.5, -1.5) {$\scriptstyle Q(s,a)$};
    \node at (4.5, -1.5) {\scriptsize Cheese};
    \foreach \x in {0.5, ..., 2.5} {
      \draw[->] (\x,-1.5) -- (\x-0.125,-1.5);
      \draw[->] (\x,-1.5) -- (\x+0.125,-1.5);
    }
    \draw[fill,purple!50] (3, -2) rectangle (4, -1);
    \draw[->] (3.5,-1.5) -- (3.5+0.125,-1.5);
    % Transition from policy evaluation to policy improvement
    \draw[->,thick] (5.25, -1.5) -- (6, -1.5) -- (6, 0.5) -- (6.75, 0.5);
    % Policy improvement
    \draw[fill,gray!50] (8,0) rectangle (13, 1);
    \draw (8,0) grid (13, 1);
    \node at (7.5, 0.5) {$\pi$};
    \node at (12.5, 0.5) {\scriptsize Cheese};
    \foreach \x in {8.5, ...,10.5} {
      \draw[->] (\x,0.5) -- (\x-0.125,0.5);
      \draw[->] (\x,0.5) -- (\x+0.125,0.5);
    }
    \draw[->] (11.5, 0.5) -- (11.5+.125,0.5);
    % Continual updates...
    \node at (10.5, -0.5) {$\vdots$};
    % Convergence to optimal policy pi^*
    \draw[fill,gray!50] (8,-2) rectangle (13, -1);
    \draw (8,-2) grid (13, -1);
    \node at (7.5, -1.5) {$\pi^*$};
    \node at (12.5, -1.5) {\scriptsize Cheese};
    \foreach \x in {8.5, ...,11.5} {
      \draw[->] (\x,-1.5) -- (\x+0.125,-1.5);
    }
  \end{tikzpicture}
  \caption{\footnotesize Suppose that we have a four-state corridor with cheese at the end. A mouse, our agent, starts out knowing nothing and follows a random policy. Eventually, the mouse will stumble into the cheese simply by moving randomly, and it is as that point that the episode concludes, and with Monte Carlo GPI we update action values $Q(s,a)$. Then, it improves its policy by greedifying with respect to the action values. This process then repeats, eventually learning the optimal policy.}
\label{fig: cheeseonaunivariategridwithamouseexample}
\end{figure}
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node[draw,circle] (st) at (0, 0) {$S_{t}$};
    \node[draw,circle,scale=1/4,fill] (at) at (1.5, 0) {};
    \foreach \x in {1, 2, 3} {
      \node[draw,circle,scale=3/4] (s\x) at (\x*3, 0) {$S_{t+\x}$};
      \node[draw,circle,scale=1/4,fill] (a\x) at (\x*3 + 1.5, 0) {};
    }
    \node (leftdots) at (-2, 0) {$\ldots$};
    \draw (leftdots) -- (st);
    \draw (st) -- (at);
    \draw (at) -- (s1);
    \draw (s1) -- (a1);
    ...
    \draw (a2) -- (s3);
    \draw (s3) -- (a3);
    \node[above right = 1pt of at] {$R_{t+1}$};
    \node[below  left = 1pt of at] {$A_{t}$};
    \node[above right = 1pt of a1] {$R_{t+2}$};
    \node[below  left = 1pt of a1] {$A_{t+1}$};
    \node[above right = 1pt of a2] {$R_{t+3}$};
    \node[below  left = 1pt of a2] {$A_{t+2}$};
    \node[below  left = 1pt of a3] {$A_{t+3}$};
    \node[right = 1mm of a3] {$\ldots$};
  \end{tikzpicture}
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}[scale=3/4]
    \node at (9.5, 7.5) {$\color{orange} \pi^*$};
    \node at (10.25, 7.5) {$\color{orange!50} + \epsilon$};
    \draw (0, 0) grid (10, 7);
    \node (s) at ( 0.5, 3.5 ) {S};
    \node (g) at ( 7.5, 3.5 ) {G};
    \foreach \x in {0.5, 1.5, 2.5} {
      \node at (\x, -0.5) {\color{gray!80} \scriptsize 0};
    }
    \foreach \x in {3.5, 4.5, 5.5} {
      \node at (\x, -0.5) {\color{gray!90} \scriptsize 1};
    }
    \foreach \x in {6.5, 7.5} {
      \node at (\x, -0.5) {\scriptsize 2};
    }
    \node at (8.5, -0.5) {\color{gray!90} \scriptsize 1};
    \node at (9.5, -0.5) {\color{gray!80} \scriptsize 0};
    \draw[->,ultra thick,orange!80] (s) -- (3.5, 3.5) -- (6.5, 6.5) -- (9.5,     6.5) -- (9.5, 2.5) -- (8.5, 2.5) -- (g);
    \draw[->, ultra thick, orange!20] (2.5, 3.5) -- (2.5, 2.5);
    \draw[->, ultra thick, orange!20] (4.5, 4.5) -- (3.5, 5.5);
    \draw[->, ultra thick, orange!20] (5.5, 5.5) -- (5.5, 6.5);
    \draw[->, ultra thick, orange!20] (9.5, 4.5) -- (8.5, 4.5);
  \end{tikzpicture}
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node[draw,circle,fill,scale=1/2] (root) at (0, 0) {};
    \node[draw,circle,scale=3/4] (state) at (0, -1) {};
    \draw[->] (root) -- (state);
    \foreach \x in {-1, 0, 1} {
      \node[draw,circle,fill,scale=1/2] (a\x) at (\x, -2) {};
      \draw[->] (state) -- (a\x);
    }
    \path (-0.5, -1.5) edge[bend right] (0.5, -1.5);
  \end{tikzpicture}
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node[draw,rectangle] (model) at (0, 0) {\large Model};
    \node (state) at (-2, 0.5) {State};
    \node (action) at (-2, -0.5) {Action};
    \node (reward) at (2, 0.5) {Reward};
    \node (nextstate) at (2.25, -0.5) {Next state};
    \draw[->] (state) -- (model);
    \draw[->] (action) -- (model);
    \draw[->] (model) -- (reward);
    \draw[->] (model) -- (nextstate);
  \end{tikzpicture}
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node (mdl) at (-5, 0) {Model};
    \node (sim) at (-2, 0) {$\substack{\textrm{Simulated} \\         \textrm{Experience}}$};
    \draw[->] (mdl) -- (sim);
    \node (vf) at (+1, 0) {$\substack{\textrm{Value} \\ \textrm{Function}}$};
    \draw[->] (sim) -- (vf);
    \node (po) at (+4, 0) {Policy};
    \draw[->] (vf) -- (po);
    \node at (-3.5, 0.25) {\small creates};
    \node at (-0.5, 0.25) {\small updates};
    \node at (+2.5, 0.25) {\small modifies};
  \end{tikzpicture}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \node (policy) at (4, 0) {policy};
    \draw[->, ultra thick] (model) -- (policy);
    \node at (2, 0.25) {planning};
  \end{tikzpicture}
\end{figure}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    \node (model) at (0, 0) {model};
    \node (simexp) at (4, 0) {$\substack{\textrm{simulated}\\         \textrm{experience}}$};
    \node (values) at (8, 0) {values};
    \node (policy) at (12,0) {policy};
    \draw[->, ultra thick] (model) -- (simexp);
    \draw[->, ultra thick] (simexp) -- (values);
    \draw[->, ultra thick] (values) -- (policy);
    \node at (6, 0.25) {\tiny backups};
  \end{tikzpicture}
\end{figure}
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node (vp) at (0, 0) {value/policy};
    \node (exp) at (2, -2) {experience};
    \node (mdl) at (-2,-2) {model};
    \draw[->, ultra thick] (vp) edge[bend left] node[right] {acting} (exp);
    \draw[->, ultra thick] (exp) edge[bend left] node[below] {$\substack{\textrm{model} \\ \textrm{learning}}$} (mdl);
    \draw[->, ultra thick] (mdl) edge[bend left] node[left] {planning} (vp);
    \draw[->, ultra thick] (exp) edge[bend left] node[right] {$\substack{\textrm{direct} \\ \textrm{RL}}$} (vp);
\end{tikzpicture}
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node[draw,rectangle] (env) at (0, -2) {Environment};
    \node[draw,rectangle] (pvfnc) at (0, 2) {Policy/value functions};
    \node[draw,rectangle] (mdl) at (3, -1.5) {Model};
    \node[draw,rounded rectangle] (realexp) at (0, 0) {$\substack{\textrm{real}         \\ \textrm{experience}}$};
    \draw[->, ultra thick, blue!50, line width = 2mm] (realexp) -- (pvfnc); 
    \draw[->, ultra thick, blue!50, line width = 2mm] (realexp) -- (env); 
    \node[draw,rounded rectangle] (simexp) at (3, 0.5)         {$\substack{\textrm{simulated} \\ \textrm{experience}}$};
    \node (directrlupdate) at (-3, 0.5) {$\substack{\textrm{direct RL} \\         \textrm{update}}$};
\draw[->] (realexp) -- (mdl);
\node at (1.65, -0.5) {\scriptsize model};
\node at (2.1, -0.75) {\scriptsize learning};
\draw[->] (mdl) -- (simexp);
\node at (3.5, -0.5) {$\substack{\textrm{search} \\ \textrm{control}}$};
\draw[->] plot [smooth] coordinates { (-1, 0) (-2, 0) (-2, 1) (-1, 1.5)};
\draw[->] plot [smooth] coordinates { ( 2, 0.5) (1.75, 0.5) (1, 1.5)};
\node at (2.5, 1.25) {\scriptsize planning update};
\end{tikzpicture}
...
\For{ever} {
  ...
  $Q(S,A) \gets Q(S,A) + \alpha \left[ R + \gamma \max_a Q(S', a) -     Q(S,A)\right]$ \tcp{Direct RL}
$\texttt{Model}(S,A) \gets R, S'$ (assuming deterministic environment) \tcp{Model-learning}
\For{$i=1,\ldots,n$}{
  \tcp{Planning...}
  $S \gets $ random previously observed state \hspace{105pt}\tcp{Search...}
  $A \gets $ random action previously taken in $S$ \hspace{88pt}\tcp{...Control}
  $R, S' \gets \texttt{Model}(S,A)$   \hspace{196pt}\tcp{Model Query}
  $Q(S,A) \gets Q(S,A) + \alpha \left[ R + \gamma \max_a Q(S', a) - Q(S,A)\right]$    \hspace{10pt} \tcp{Value Update}
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \draw[fill,gray!50] (2, 2) rectangle (3, 5);
    \draw[fill,gray!50] (5, 1) rectangle (6, 2);
    \draw[fill,gray!50] (7, 3) rectangle (8, 6);
    \node at (0.5, 3.5) {S};
    \node at (8.5, 5.5) {G};
    \draw[step=1.0,black,thin] (0,0) grid (9, 6);    
  \end{tikzpicture}
...
\begin{align*}
  V(S_t) \gets V(S_t) + \alpha \left[ \hat G_t - V(S_t)\right] \leadsto Q(S_t, A_t) \gets Q(S_t, A_t) + \alpha \left[ \hat G_t - Q(S_t, A_t)\right]
\end{align*}
where we can define a variety of update rules:
\begin{align*}
  &\textrm{Sarsa} &\hat G_t &= R_{t+1} + \gamma Q(S_{t+1}, A_{t+1}) \\
  &\textrm{Q-Learning} &\hat G_t &= R_{t+1} + \gamma \max_{a'} Q(S_{t+1}, a') \\
  &\textrm{Expected Sarsa} &\hat G_t &= R_{t+1} + \gamma \sum_{a'} \pi(a'|S_{t+1}) Q(S_{t+1}, a')
\end{align*}
