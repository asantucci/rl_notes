...
On the other hand, is it really necessary to wait until the final outcome is known before learning can begin?  Perhaps if we get stuck in traffic along our way home, we can increase our estimate for our initial state before waiting to actually get home. The TD approach suggests that each error is proportional to the change over time of the prediction, i.e. to the \emph{temporal differences} in predictions.
...
Consider the following \emph{Markox reward process}, which is a Markov decision process without actions.
