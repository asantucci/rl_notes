...
\tcp{Policy Evaluation (really a \texttt{do-while} loop).}
\While{$\Delta \geq \theta$} {
  $\Delta \gets 0$ \\
  \For{$s \in \mathcal S$} {
    $v \gets V(s)$ \\
...
\If{\texttt{old\_action} $\neq \pi(s)$}{
  \texttt{policy\_stable} $\gets \texttt{false}$
}
}
\If{\texttt{policy\_stable}} {
...
\end{equation}
for all $s \in \mathcal S$. For any arbitrary $v_0$, the sequence $\{v_k\}$ can be shown to converge to $v_*$ under the same conditions that guarantee the existence of $v_*$.
